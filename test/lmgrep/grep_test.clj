...
(deftest handle-no-split-option
  (let [file "test/resources/test.txt"
        ; both terms are required for a match
        query "+dog +Apache"]
    (testing "there is no match when matching terms are in separate lines"
      (let [options {:split    true
        ...
        (is (str/blank?
              (str/trim
                (with-out-str
                  (grep/grep [query] file nil options)))))))
    (testing "there IS A match when matching terms are in separate lines"
