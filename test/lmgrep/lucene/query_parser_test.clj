...
(deftest simple-query-parser-creation
  (testing "configuration differences"
    (let [config {:flags 123
          ...
          default-qp (qp/simple-qp empty-config field-name analyzer)
          qp (qp/simple-qp config field-name analyzer)]
...
(deftest classic-qp-creation
  (testing "empty configuration"
    (let [config {}
          query-parser (qp/classic-qp config field-name analyzer)]
  ...
  (testing "query-parser-base config"
    (let [config {:max-determinized-states 123}
          default-qp (qp/classic-qp empty-config field-name analyzer)
          qp (qp/classic-qp config field-name analyzer)]
